...
class change_filter:  # noqa: N801,N806 pylint: disable=invalid-name
    ...
    def validate(self) -> None:
        ...
        """
        if self._option not in configdata.DATA and not configdata.is_valid_prefix(
            self._option
        ):
...
class KeyConfig:
    ...
    def get_command(
        self, key: keyutils.KeySequence, mode: str, default: bool = False
    ) -> Optional[str]:
...
class Config(QObject):
    ...
    def __init__(
        self, yaml_config: "configfiles.YamlConfig", parent: QObject = None
    ) -> None:
    ...
    def _set_value(
        ...
        hide_userconfig: bool = False,
    ) -> None:
        """Set the given option to the given value."""
        if not isinstance(objects.backend, objects.NoBackend):
            if objects.backend not in opt.backends:
                raise configexc.BackendError(
                    opt.name, objects.backend, opt.raw_backends
                )

        opt.typ.to_py(value)  # for validation

        self._values[opt.name].add(
            opt.typ.from_obj(value), pattern, hide_userconfig=hide_userconfig
        )

        self.changed.emit(opt.name)
        log.config.debug("Config option changed: {} = {}".format(opt.name, value))
    ...
    def read_yaml(self) -> None:
        ...
        for values in self._yaml:
            for scoped in values:
                self._set_value(values.opt, scoped.value, pattern=scoped.pattern)
    ...
    def get(self, name: str, url: QUrl = None, *, fallback: bool = True) -> Any:
    ...
    def get_obj(self, name: str, *, url: QUrl = None, fallback: bool = True) -> Any:
    ...
    def get_obj_for_pattern(
        self, name: str, *, pattern: Optional[urlmatch.UrlPattern]
    ) -> Any:
        """Get the given setting as object (for YAML/config.py).
    ...
    def get_mutable_obj(self, name: str, *, pattern: urlmatch.UrlPattern = None) -> Any:
    ...
    def get_str(self, name: str, *, pattern: urlmatch.UrlPattern = None) -> str:
    ...
    def set_obj(
        ...
        *,
        ...
        self._set_value(opt, value, pattern=pattern, hide_userconfig=hide_userconfig)
        if save_yaml:
    ...
    def set_str(
        ...
        *,
        ...
        log.config.debug(
            "Setting {} (type {}) to {!r} (converted from {!r})".format(
    ...
    def unset(
        self, name: str, *, save_yaml: bool = False, pattern: urlmatch.UrlPattern = None
    ) -> bool:
    ...
    def __getattr__(self, attr: str) -> Any:
        ...
        with self._handle_error(f"getting '{name}'"):
            if self._configapi is None:
                ...
                return self._config.get_mutable_obj(name, pattern=self._pattern)
